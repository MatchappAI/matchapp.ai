...
| Brand/contact ingestion | Manual entry + CSV import | Active | Seed brands and contacts | No chosen discovery/enrichment provider yet. |
| Optional enrichment | Apify / Hunter-style adapters | Not selected | Future-only provider integration | Keep behind an adapter and honest “not configured” states. |
...
4. Treat wallet / escrow / payout / Connect code paths as legacy compatibility
   surfaces only unless the product direction changes explicitly.
5. Do not introduce a new provider dependency without a product decision.
